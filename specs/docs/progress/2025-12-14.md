...
- ISO-8601 timestamps
- One file per document: `anchors_[documentId].json` (array of anchors)
...
- `GET /api/anchors?documentId=...` - List anchors
...
- ✅ Anchors load on refresh; selecting an anchor scrolls to the correct page
- ✅ Deleting a note does not delete the anchor (by design)

### Persistence
- ✅ Anchor data persists to Google Drive
- ✅ Metadata stored in `/Vinculum_Data/Metadata/`
- ✅ Reloading app restores library, anchors, and notes per selected document
...
1. **Text Extraction**: Selections with no text fall back to a placeholder quote
   - User is prompted to provide a label for these anchors
   - Resolution: Improve extraction for border/margin selections

2. **Markdown Rendering**: Non-PDF documents show a placeholder view
   - Markdown viewer is not yet implemented
...
#### 1. Type Definitions & Schema Extensions ✅
...
2. Build chunk_id → chunk map
3. For each chunk: search PDF for text location → create Anchor
4. For each alignment: resolve chunk_id refs → create Alignment
...
- ≤20px drift constraint enforcement
...
- ✅ Text search without rect coordinates (fuzzy matching + fallback)
- ⏳ Sync scroll drift constraint (≤20px) - algorithm designed, not implemented
- ⏳ JSONL chunk-to-anchor mapping - parser complete, API integration pending
